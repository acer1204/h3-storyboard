...
- **慣性偵測（自動，第 1 層）**：掃描近期紀錄，找出「跨許多**不同**圖片」重複出現的動作片語——那些不是從圖片來的，是模型的慣性。生成時自動注入避開清單（除非圖片真的需要）。硬切／一鏡到底分開統計，各取每張圖的最新版本、最多 30 張、門檻 30%；重疊片段會沿文本還原成一條完整慣性句。系統設定可開關、可看目前清單。
