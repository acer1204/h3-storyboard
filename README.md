...
- **警告（預設只顯示不重試；重試目標選「完全通過」才會重試）**：`medium` / `moderate` 沒省略、非官方運鏡動詞（drift…）、`large amplitude` 配非 push/zoom 運鏡、音樂抽象情緒詞或解釋情緒功能、角色聽得到的音樂寫進 `non_diegetic_music`、`<d>` 內夾語氣動作、非旁白卻寫唇句、缺風格宣告、缺人數鎖定句等。
...
模型會不看圖硬套慣性劇情（每張圖都「轉頭面向鏡頭微笑」）。對策分層：

- **慣性偵測（僅提醒）**：掃描近期紀錄，找出「跨許多**不同**圖片」重複出現的**動作**與**運鏡**——那些不是從圖片來的，是模型的慣性。偵測結果只顯示在系統設定，**不自動影響生成**；想抑制哪一條，按「轉成規則」放進經驗庫（可編輯/停用）才會注入。硬切／一鏡到底分開統計，各取每張圖的最新版本、最多 30 張、門檻 30%。「整理經驗」時統計會作為旁證附給模型，但只有與你的反饋互相印證才可立規則。
- **評分與備註**：歷史紀錄詳情多了 👍／👎、原因標籤（劇情不合理／動作突兀／畫面模糊…）與一句話備註——看完 ComfyUI 影片後評，卡片會顯示評分標記。
- **經驗庫（第 2 層）**：反饋累積後（建議約 5 筆）按系統設定的「整理經驗」，模型會一次歸納全部待整理的反饋、提出最多 3 條通用規則（一次性反饋自動忽略）；你審核、改字、選模式後入庫。入庫規則每次生成都注入（依模式、最多 15 條），可隨時停用／刪除，也可以手動新增（中文自動翻英）。重新評分過的紀錄會重新列入待整理。

### 🎛️ ComfyUI 設定頁（v1.2）

- 左側選單新增「ComfyUI 設定」：模板管理（抓最近一次成功生成／上傳 Export (API) 檔）搬到這裡，並新增**工作流預設參數**——FPS、解析度（DaSiWa 計算機的完整清單）、步數、shift_video、shift_audio。留空＝照模板原值，填了就在每次送出時覆寫對應節點。
- 「上傳並處理」卡片改版：補充劇情移到右欄檔名上方、輸入時自動長高（不用捲）；原位置改為**輸出比例**設定——下拉 1:1／2:3／3:4／5:8／9:16／9:21（直式 寬:高）＋手動數值＋直式/橫式切換，**預設依上傳圖片解析度自動匹配最接近的比例**。歷史紀錄重送時伺服器端同樣自動依原圖匹配。

### 其他

- 一鍵送 ComfyUI：模板 = ComfyUI 最近一次成功的生成，**或直接上傳 Export (API) 匯出的 workflow JSON**；之後每次只替換圖片、三個欄位與影片秒數
...
- **影片時長 / 重試**：一鏡到底 3 階段建議 `15 seconds`；重試目標預設「無錯誤即可」（警告只顯示）；想連警告都消掉再選「完全通過」。
