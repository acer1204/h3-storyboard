...
- **上傳媒體庫**：你上傳過的每張原圖（同內容只留一份，以 hash 去重），標示被幾筆紀錄使用。多選 → 一鍵匯回「上傳並處理」。刪除時可選「只從庫移除（保留歷史）」或「連同歷史一起刪」；同一張圖日後再上傳，舊紀錄自動重新掛回。
...
- 一鍵送 ComfyUI：模板 = ComfyUI 最近一次成功的生成，之後每次只替換圖片與三個欄位
...
```
history/    每筆生成（含原圖副本、縮圖）
uploads/    上傳庫（以內容 hash 去重）
loras/      LoRA 預設
